...
export async function detectAdSlots(page) {
    const results = [];
    const viewport = page.viewportSize() ?? { width: 1280, height: 800 };
    const viewportArea = viewport.width * viewport.height;
    const addIfAdFrame = async (frameElement) => {
        try {
            ...
            const isKeyword = KEYWORDS.some((k) => meta.includes(k) || srcLower.includes(k));
            const isKnownIframe = AD_IFRAME_PATTERNS.some((pattern) => srcLower.includes(pattern) || meta.includes(pattern));
            if (!isAdNetwork && !isKeyword && !isKnownIframe)
                return;
...
//# sourceMappingURL=ad-detector.service.js.map